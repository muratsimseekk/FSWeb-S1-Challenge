...
console.log("Görev 1-2 : ", fenomenler[0].followers);
...
function indekseGoreFenomen(diziAdi, indexNo) {
  let fenomenAdi = diziAdi[indexNo].profile;

  return `${indexNo}. indekste bulunan fenomen: ${fenomenAdi}`;
}
...
function profilListesi(fenomenDizisi) {
  let kopyaDizi = [...fenomenDizisi];

  const profiller = [];

  for (let i = 0; i < kopyaDizi.length; i++) {
    profiller.push(kopyaDizi[i].profile);
  }
...
function fenomenSil(alinacakDizi, diziIndexNo) {
  let yeniKopyaDizi = [...alinacakDizi];

  yeniKopyaDizi.splice(diziIndexNo, 1);

  return yeniKopyaDizi;
}
...
function fenomenEkle(
  ...
  let baskaKopya = [...diziIsmi];

  const obj = {};

  obj.number = siraNo;
  obj.profile = profilIsmi;
  obj.followers = takipciSayisi;
  obj.posts = postSayisi;
  obj.platform = platformAdi;
...
function enFenomenler(arrName) {
  let fazlaTakipcililer = [];

  for (let i = 0; i < arrName.length; i++) {
    let takipci = arrName[i].followers;

    if (takipci > 100000000) {
      fazlaTakipcililer.push(arrName[i].profile);
    }
  }
  return fazlaTakipcililer;
}
...
function fenomenGonderimSayisi(diziName, profileAdi) {
  for (let i = 0; i < diziName.length; i++) {
    if (diziName[i].profile == "Will Smith") {
      return diziName[i].posts;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(fenDizi, platAdi) {
  let yorumArr = [...fenDizi];

  let tiktokDizi = [];

  let numbers = [];
  let maxNumber;
  for (let i = 0; i < yorumArr.length; i++) {
    if (yorumArr[i].platform == platAdi) {
      tiktokDizi.push(yorumArr[i]);
    }
  }
  for (let y = 0; y < tiktokDizi.length; y++) {
    numbers.push(tiktokDizi[y].posts);
  }

  maxNumber = Math.max(...numbers);

  for (let x = 0; x < yorumArr.length; x++) {
    if (maxNumber == yorumArr[x].posts) {
      return yorumArr[x].profile;
    }
  }
}
